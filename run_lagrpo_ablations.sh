...
python train_grpo.py --model-name "$MODEL" --ablation B1 --group-size $G --max-steps $STEPS --sft-path $SFT --exp-id ablation_B1 --batch-size 2 --accum-steps 1 --max-new-tokens 512

# 3. B2: + Step Annealing (Time Dimension)
echo ""
echo "── Step 3: B2 Time (Reward Annealing) ──"
python train_grpo.py --model-name "$MODEL" --ablation B2 --group-size $G --max-steps $STEPS --sft-path $SFT --exp-id ablation_B2 --batch-size 2 --accum-steps 1 --max-new-tokens 512

# 4. B3: + Adv Clipping (Variance Dimension)
echo ""
echo "── Step 4: B3 Variance (Advantage Clipping) ──"
python train_grpo.py --model-name "$MODEL" --ablation B3 --group-size $G --max-steps $STEPS --sft-path $SFT --exp-id ablation_B3 --batch-size 2 --accum-steps 1 --max-new-tokens 512
